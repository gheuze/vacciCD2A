...
png("sorties/dtp/evolution_CV_nat-dep-PMI%02d.png",width=25 ,height=15, units="cm",res = 400)
# figure sur les primovacci, cad lignes impaires du df, soit graphe[c(T,F),]
      matplot(t(graphe[c(T,F),]),type = "b",pch = c(rep(1,2),3), xlab = "années", ylim = c(75,100),ylab = "taux de couverture vaccinal en %",
               main = "Comparaison de l'évolution de la primovaccination DTP
...
graphe <- read.csv2("sorties/dtp/dtp_24-4.csv",header = T)
...
png("sorties/dtp/evolution_CV_24-4.png",width=25 ,height=15, units="cm",res = 400)
matplot(t(graphe),type="b",pch = c(3,4,2,6), xlab = "année de naissance de l'enfant", ylim = c(75,100),ylab = "taux de couverture vaccinal en %",
         main = "Evolution de la couverture vaccinale DTP à l'âge de 24 mois et 4 ans,
        pour les enfants nés entre 1993 et 2011 - axe des ordonnées débutant à 75 %", xaxt="n", col = couleurs, lty = 3)
axis(1,at=1:19,labels=1993:2011)
legend(15,80,legend = rownames(graphe), pch = c(3,4,2,6),col = couleurs, cex=0.8,bty="n",lwd=1,lty = 3)
legend(15,76,legend = "objectif de vaccination", pch="",col = "darkorange", cex=0.8,bty="n",lwd=1,lty = 2)
abline(h = 95, col = "darkorange",lty = 2)
dev.off()
