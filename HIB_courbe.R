...
graphe <- read.csv2("sorties/hib/hib - pour graphe.csv",header = T)
...
# figure sur les primovacci, cad lignes impaires du df, soit graphe[c(T,F),]
      ...
      legend(15,55,legend = rownames(graphe[c(T,F),]), pch = c(1,1,3), col = couleurs, cex=0.8,bty="n",lwd=1,lty = c(1,rep(3,2)))
...
# figure sur les rappels, cad lignes paires du df, soit graphe[c(F,T),]
      ...
      à l'âge de 24 mois suivant les sources, 1995-2015 - axe des ordonnées débutant à 45 %", xaxt="n",lty = c(1,rep(3,2)),
              col = couleurs)
      axis(1,at=1:21,labels=1995:2015)
      legend(15,55,legend = rownames(graphe[c(F,T),]), pch = c(5,5,4), col = couleurs, cex=0.8,bty="n",lwd=1,lty = c(1,rep(3,2)))
      legend(15,48,legend = "objectif de vaccination", pch="",col = "darkorange", cex=0.8,bty="n",lwd=1,lty = 2)
      abline(h = 95, col = "darkorange",lty = 2)
dev.off()
...
rownames(graphe) <- c("primovaccination à 24 mois", "rappel à 24 mois",
                      "primovaccination à 4 ans", "rappel à 4 ans")
couleurs <- c("blue")
