...
#########################################################################
# import et creation d'un df unique
#########################################################################
...
##############
#####################################################################
# creation de la fonction calcul CV pour tempo_dtp
f_dtp <- function (df_dtp,an){
      ############################################################
      # preparation
      ...
      tempo_population <- comptage(tempo_dtp,an,3) # calcul population denominateur avant restriction sur vaccines
      ...
      sortie <- list(
            ("DTP, 1iere dose, resume des ages"), summary(as.numeric(tempo_dtp_1dose$age_vacci_mois[tempo_dtp_1dose$acte_code == "P1"])),
            ("DTP, 2ieme dose, resume des ages"), summary(as.numeric(tempo_dtp_2dose$age_vacci_mois[tempo_dtp_2dose$acte_code == "P2"])),
            ("DTP, 3ieme dose, resume des ages"), summary(as.numeric(tempo_dtp_3dose$age_vacci_mois[tempo_dtp_3dose$acte_code == "P3"])),
            ("DTP, 4ieme dose, resume des ages"), summary(as.numeric(tempo_dtp_4dose$age_vacci_mois[tempo_dtp_4dose$acte_code == "R01"])),
            ("DTP, CV, 1 dose"), dim(tempo_dtp_1dose)[1] / tempo_population * 100,
            ("DTP, CV, 2 doses"), dim(tempo_dtp_2dose)[1] / tempo_population * 100,
            ("DTP, CV, 3 doses"), dim(tempo_dtp_3dose)[1] / tempo_population * 100,
            ("DTP, CV, 4 doses"), dim(tempo_dtp_4dose)[1] / tempo_population * 100
     )
      return(sortie)
}
########################
